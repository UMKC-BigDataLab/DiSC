...
NODS=16
FPATH=/dev/data
FAMS=$FPATH/syn-families.txt
VARS=$FPATH/syn-variables.txt
TABL=$FPATH/syn1.table.txt
...
for R in 40 80 120; do
  for K in 1 2 3; do
    ...
    sed -i 's/DPF/1/g'   ~/target/configuration/config.txt
    sed -i 's/IES/70000/g' ~/target/configuration/config.txt
